...
There are 9 different examples that can be choose in order to verify the proper operation of the code.
...
def demo_examples(n_example):
    ...
    A, b, c, min_or_max, constr_signs = parse_json("test_data.json", n_example)
    model = Linear_Model(A, b, c, min_or_max, constr_signs,
                         rule="standard", verbose=True)
    model.optimization()
...
def demo_diet(objective):
    ...
    if objective == 1:
        ...
        model = Linear_Model(A, b, c, "MIN", constr_signs,
                             rule="blands", verbose=False)
        model.optimization()
    ...
    if objective == 2:
        ...
        model = Linear_Model(A, b, c, "MAX", constr_signs,
                             rule="blands", verbose=False)
        model.optimization()
...
# Select the example inserting a number between 1 and 9
# demo_examples(n_example="10")
